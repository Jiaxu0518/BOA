...
private:
	...
	int time;  //timeline长度
public:
	SegVarOpt(double budget, int n, int time)
	{
		this->budget = budget;
		this->n = n;
		this->time = time;
	...
	void test(Info &item)
	{
		...
		//获得最优数组
		if (n != SEGS) { cout << "n和SEGS分段数量不匹配" << endl; return; }
		...
		for (int i = 1; i <= n; i++) //每一段统计一次
		{
			...
			buildGraph(WORKER_NUM, TASK_NUM,i,threshold);
			//matching
			//cout << "mathcing..." << endl;
			...
			removeSEGAvail(m);
			//cout << "flow: " << flow << "vector size:" << mat.size() << endl;
		...
		Util::setInfo(item, "SegVarOpt", (double)fcost, (double)(finish - start) / CLOCKS_PER_SEC, cos.WorkingSetSize / 1024, mat.size(), n);
	...
	void buildGraph(int workerNum, int taskNum, int sid, double threshold[])
	{
		...
		//cout << "在空图基础上构建新图,threshold选择为:" << threshold[sid-1]<< endl;
		//在空图基础上构建新图
		...
		for (vector<int>::iterator itt = avail_tasks.begin(); itt != avail_tasks.end(); itt++)
			for (vector<int>::iterator itw = avail_workers.begin(); itw != avail_workers.end(); itw++)
			{
				...
				if (fabs(dis + 1)>eps && Util::greater(threshold[sid-1], dis, 1)) //worker can arrive before deadline
				{
