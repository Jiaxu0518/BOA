...
#define RTREE_USE_SPHERICAL_VOLUME // Better split classification, may be slower on some systems
	...
	public:
		...
		bool KNN(const ELEMTYPE coord[NUMDIMS], const int k_value, std::vector<DATATYPE>& result, int workerId, double &dis, double threshold, ELEMTYPEREAL dis_limit = -1, t_resultCallback predicate = nullptr, void* a_context = nullptr);
	...
	protected:
		...
		ELEMTYPEREAL CalcPointRectDis_GPS_Manhattan(const ELEMTYPE coord[NUMDIMS], const Rect* a_rect);
		ELEMTYPEREAL CalcPointRectDis_GPS_Euclidean(const ELEMTYPE coord[NUMDIMS], const Rect* a_rect);
	...
	RTREE_TEMPLATE
		bool RTREE_QUAL::KNN(const ELEMTYPE coord[NUMDIMS], const int k_value, std::vector<DATATYPE>& result, int workerId, double &dis, double threshold, ELEMTYPEREAL dis_limit, t_resultCallback predicate, void* a_context)
	{
		...
		while (node)
		{
			for (int i = 0; i < node->m_count; i++)
			{
				...
				if (branch)
				{
					ELEMTYPEREAL distance;
					if (mode)//syn
					{
						distance = CalcPointRectDis(coord, &branch->m_rect);//返回值
					}
					else  //real
					{
						distance = CalcPointRectDis_GPS_Euclidean(coord, &branch->m_rect);//返回值
					}
					if (dis_limit < 0 || distance < dis_limit)
					{
						if (Util::greater(threshold, distance, 1))
			...
			while (knnode_queue.size())
			{
				...
				if (knode.isLeaf())
				{
					...
					{
						//提取叶子节点数据
						int taskId = knode.m_branch->m_data;
						dis = knode.m_distance;
						if (mode) //syn
						{
							if (Util::greater(task[taskId - 1].deadline, worker[workerId - 1].appear + dis, 1))
						...
						else //real
						{
							...
							if (Util::greater(task[taskId - 1].deadline, worker[workerId - 1].appear + time, 1))
							{
								//cout << "该叶子正是要找的"<<endl;
								result.push_back(taskId);
							}
	...
	RTREE_TEMPLATE
		ELEMTYPEREAL RTREE_QUAL::CalcPointRectDis_GPS_Manhattan(const ELEMTYPE coord[NUMDIMS], const Rect* a_rect)
	{
		ELEMTYPEREAL distance = (ELEMTYPEREAL)0;
		//经度 coord[0],a_rect->m_min[0],a_rect->m_max[0]表示经度
		if (coord[0] < a_rect->m_min[0])
			distance += Util::Euclidean_Real(coord[0], coord[1], a_rect->m_min[0], coord[1]);
		else if (coord[0] > a_rect->m_max[0])
			distance += Util::Euclidean_Real(coord[0], coord[1], a_rect->m_max[0], coord[1]);
		else
			distance += 0;
		//纬度 coord[1],a_rect->m_min[1],a_rect->m_max[1]表示纬度
		if (coord[1] < a_rect->m_min[1])
			distance += Util::Euclidean_Real(coord[0], coord[1], coord[0],a_rect->m_min[1]);
		else if (coord[1] > a_rect->m_max[1])
			distance += Util::Euclidean_Real(coord[0], coord[1], coord[0], a_rect->m_max[1]);
		else
			distance += 0;
		return distance;	
	}
	...
	RTREE_TEMPLATE
		ELEMTYPEREAL RTREE_QUAL::CalcPointRectDis_GPS_Euclidean(const ELEMTYPE coord[NUMDIMS], const Rect* a_rect)
	{
		ELEMTYPEREAL distance = (ELEMTYPEREAL)0;
		double posx, posy;
		//经度 coord[0],a_rect->m_min[0],a_rect->m_max[0]表示经度
		...
		if (coord[1] < a_rect->m_min[1])
			...
			posy = coord[1];
		distance = Util::Euclidean_Real(coord[0],coord[1],posx,posy);
		return distance;
	}
