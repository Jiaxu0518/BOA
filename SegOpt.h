...
private:
	...
	int time;  //timeline长度
public:
	SegOpt(double budget,int n, int time)
	{
		this->budget = budget;
		this->n = n;
		this->time = time;
	...
	void test(Info &item)
	{
		...
		for (int i = 1; i <=n; i++) //每一段统计一次
		{
			...
			//可以用工人和任务
			searchSEGAvail_r(i);
			//cout << "第" << i << "时间段可用工人数：" << avail_workers.size() << "可用任务数：" << avail_tasks.size();
		...
		Util::setInfo(item, "SegOPT", (double)fcost, (double)(finish - start) / CLOCKS_PER_SEC, cos.WorkingSetSize / 1024, mat.size(),n);
	...
	void buildGraph(int workerNum, int taskNum)
	{
		...
		for (vector<int>::iterator itt = avail_tasks.begin(); itt != avail_tasks.end(); itt++)
			for (vector<int>::iterator itw = avail_workers.begin(); itw != avail_workers.end(); itw++)
			{
				...
				if (fabs(dis + 1)>eps) //worker can arrive before deadline
				{
