...
private:
	...
	double threshold;
public:
	SegFixOpt(double budget, int n, int time,double threshold)
	{
		this->budget = budget;
		this->n = n;
		this->time = time;
	...
	void test(Info &item)
	{
		...
		Util::setInfo(item, "SegFixOpt", (double)fcost, (double)(finish - start) / CLOCKS_PER_SEC, cos.WorkingSetSize / 1024, mat.size(),n);
