...
private:
	...
	int time;
public:
	MBSGA(double budget, double threshold[], double budPortion[], int n, int time)
	{
		this->budget = budget;
	...
	void test(Info& item)
	{
		...
		for (vector<pair<pair<int, int>, double>>::iterator it = mat.begin();
			...
			cost += (*it).second;
		cout << "cost:" << cost << endl;
		Util::setInfo(item, "MBSGA", cost, (double)(finish - start) / CLOCKS_PER_SEC, cos.WorkingSetSize / 1024, mat.size(), n);
	}
	//matching 
